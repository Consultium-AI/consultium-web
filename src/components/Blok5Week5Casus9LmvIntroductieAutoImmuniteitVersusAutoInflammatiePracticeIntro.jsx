import { Activity, GitBranch, Layers, Stethoscope } from 'lucide-react'

/**
 * Oefenpagina-intro voor LMV Introductie auto-immuniteit versus auto-inflammatie (oranje / LMV-kleur).
 */
const Blok5Week5Casus9LmvIntroductieAutoImmuniteitVersusAutoInflammatiePracticeIntro = () => (
    ...
    >
      ...
      <div className="relative px-5 py-4 sm:px-6 sm:py-5">
        <p className="text-xs font-semibold uppercase tracking-wider text-orange-900/85 dark:text-orange-300/90 mb-3">
          Casus 9 · Patiënt met auto-immuunziekte · LMV · Introductie auto-immuniteit versus auto-inflammatie
        </p>
        <div className="flex flex-wrap gap-2 mb-4">
          {[
            { Icon: Layers, label: 'Aangeboren vs verworven afweer' },
            { Icon: GitBranch, label: 'Continuüm · spectrum' },
            { Icon: Activity, label: 'Antigeen-afhankelijk / onafhankelijk' },
            { Icon: Stethoscope, label: 'Voorbeelden ziektebeelden' },
          ].map(({ Icon, label }) => (
            <span
              key={label}
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
          Meerkeuzevragen — antwoordopties zoals in je oefenmateriaal.
        </p>
      </div>
    </div>
  </div>
...
export default Blok5Week5Casus9LmvIntroductieAutoImmuniteitVersusAutoInflammatiePracticeIntro
